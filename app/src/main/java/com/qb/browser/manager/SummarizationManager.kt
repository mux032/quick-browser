 ...
 */
class SummarizationManager private constructor(private val context: Context) {
    ...
    companion object {
        ...
        @Volatile
        private var instance: SummarizationManager? = null
        
        fun getInstance(context: Context): SummarizationManager {
            return instance ?: synchronized(this) {
                instance ?: SummarizationManager(context.applicationContext).also { instance = it }
            }
        }
    }
    ...
    private suspend fun extractContent(html: String): String = withContext(Dispatchers.Default) {
        try {
            ...
            doc.select("script, style, nav, footer, header, aside, .ads, .comments, .sidebar, iframe, form, button").remove()
